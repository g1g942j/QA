...
describe("Продукты — К (калории): границы min=0", () => {
  ...
  const caloriesCases = [
    ...
    { value: "100.1", expectHtmlValid: true },
  ] as const;
...
describe("Продукты — БЖУ: границы по одному полю (два других = 0)", () => {
  let driver: import("selenium-webdriver").WebDriver;
  ...
  for (const { field, label } of singleFieldCases) {
    for (const row of proteinOnlyCases) {
      it(`${label} = ${row.value} (остальные БЖУ=0) → validity.valid === ${row.expectHtmlValid}`, async () => {
        const modal = new ProductModalPage(driver);
        const nameEl = await modal.nameInput();
        await nameEl.clear();
...
describe("Продукты — БЖУ: сумма (белки+жиры+углеводы)", () => {
  let driver: import("selenium-webdriver").WebDriver;
  ...
  it("сумма 99.9 — валидно, продукт создаётся", async () => {
    const products = new ProductsPage(driver);
    const modal = new ProductModalPage(driver);
  ...
  it("сумма 100 — валидно, продукт создаётся", async () => {
    const products = new ProductsPage(driver);
    const modal = new ProductModalPage(driver);
  ...
  it("сумма 100.1 — клиентская ошибка «Сумма БЖУ…»", async () => {
    const modal = new ProductModalPage(driver);
    const nameEl = await modal.nameInput();
    await nameEl.clear();
